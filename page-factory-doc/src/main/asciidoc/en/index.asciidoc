...
Page Factory 2 includes large number of standart steps, so one could start developing automated tests out of the box. Page Factory is a cross-platform framework allowing to launch tests using most of popular web browsers. Also Page Factory supports iOs and Android application testing with the means of Appium.
